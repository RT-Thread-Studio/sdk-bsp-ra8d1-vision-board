...
void ra_sdhi_request(struct rt_mmcsd_host *host, struct rt_mmcsd_req *req)
{
    ...
    if (req->cmd != RT_NULL)
    {
        ...
        if (data != RT_NULL)
        {
            ...
            {
#if defined (__DCACHE_PRESENT)
                SCB_CleanInvalidateDCache_by_Addr((rt_uint32_t *)buffer, size);
#endif
...
struct rt_mmcsd_host *sdio_host_create(struct ra_sdhi *sdhi_des)
{
    ...
    memcpy(&sdio->sdhi_des, sdhi_des, sizeof(struct ra_sdhi));
...
int rt_hw_sdhi_init(void)
{
    ...
    struct ra_sdhi sdhi2;
    sdhi2.instance = &g_sdmmc2;
    sdhi2.instance->p_api->open(sdhi2.instance->p_ctrl, sdhi2.instance->p_cfg);
    host2 = sdio_host_create(&sdhi2);
    if (host2 == RT_NULL)
    {
        return -1;
    }
