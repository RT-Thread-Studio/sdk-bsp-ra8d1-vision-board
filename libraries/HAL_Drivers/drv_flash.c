...
#include <rtdbg.h>
    ...
    #define FLASH_DF_WRITE_SIZE     BSP_FEATURE_FLASH_HP_DF_WRITE_SIZE
 ...
 * @return result
 */
int _flash_write_df(rt_uint32_t addr, const rt_uint8_t *buf, size_t size)
{
    rt_err_t result      = RT_EOK;
    rt_base_t level;
    fsp_err_t err = FSP_SUCCESS;
    ...
    if (size % FLASH_DF_WRITE_SIZE)
    {
        LOG_E("Flash Write size must be an integer multiple of %d", FLASH_DF_WRITE_SIZE);
        return -RT_EINVAL;
    }
    ...
    while (written_size < size)
    {
        ...
        R_FLASH_Reset(&g_flash_ctrl);
        /* Write code flash data*/
        err = R_FLASH_Write(&g_flash_ctrl, (uint32_t)(buf + written_size), addr + written_size, FLASH_DF_WRITE_SIZE);
        rt_hw_interrupt_enable(level);
        ...
        written_size += FLASH_DF_WRITE_SIZE;
    }
...
int _flash_hp3_erase(rt_uint32_t addr, size_t size)
{
    fsp_err_t err = FSP_SUCCESS;
    rt_base_t level;
    ...
    R_FLASH_Reset(&g_flash_ctrl);
    /* Erase Block */
    err = R_FLASH_Erase(&g_flash_ctrl, RT_ALIGN_DOWN(addr, BSP_FEATURE_FLASH_HP_DF_BLOCK_SIZE), (size - 1) / BSP_FEATURE_FLASH_HP_DF_BLOCK_SIZE + 1);
    rt_hw_interrupt_enable(level);
...
static int fal_flash_hp3_read(long offset, rt_uint8_t *buf, size_t size);
static int fal_flash_hp3_write(long offset, const rt_uint8_t *buf, size_t size);
static int fal_flash_hp3_erase(long offset, size_t size);
...
{
    "onchip_flash_hp1",
    (FLASH_START_ADDRESS + BSP_FEATURE_FLASH_HP_CF_REGION0_SIZE),
...
const struct fal_flash_dev _onchip_flash_hp3 =
{
    "onchip_flash_hp3",
    BSP_FEATURE_FLASH_DATA_FLASH_START,
    BSP_DATA_FLASH_SIZE_BYTES,
    BSP_FEATURE_FLASH_HP_DF_BLOCK_SIZE,
    {
        _flash_init,
        fal_flash_hp3_read,
        fal_flash_hp3_write,
    ...
    (BSP_FEATURE_FLASH_HP_DF_WRITE_SIZE * 8)
};
...
static int fal_flash_hp3_read(long offset, rt_uint8_t *buf, size_t size)
{
    return _flash_read(_onchip_flash_hp3.addr + offset, buf, size);
}

static int fal_flash_hp3_write(long offset, const rt_uint8_t *buf, size_t size)
{
    return _flash_write_df(_onchip_flash_hp3.addr + offset, buf, size);
}

static int fal_flash_hp3_erase(long offset, size_t size)
{
    return _flash_hp3_erase(_onchip_flash_hp3.addr + offset, size);
}
