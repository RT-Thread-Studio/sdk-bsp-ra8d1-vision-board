...
__weak int sensor_reset()
{
    ...
    {
        rt_pin_write(DCMI_RESET_PIN, 1);
        rt_thread_mdelay(10);
        rt_pin_write(DCMI_RESET_PIN, 0);
    }
    else
    {
        rt_pin_write(DCMI_RESET_PIN, 0);
...
int sensor_probe_init(uint32_t bus_id, uint32_t bus_speed)
{
    ...
    // Do a power cycle
    rt_pin_write(DCMI_POWER_PIN, 1);
    rt_thread_mdelay(10);

    rt_pin_write(DCMI_POWER_PIN, 0);
    rt_thread_mdelay(10);
#endif
...
__weak int sensor_set_pixformat(pixformat_t pixformat)
{
    ...
    {
        rt_thread_mdelay(100); // wait for the camera to settle
    }
...
__weak int sensor_snapshot(sensor_t *sensor, uint8_t *image, uint32_t flags)
{
    return -1;
}
