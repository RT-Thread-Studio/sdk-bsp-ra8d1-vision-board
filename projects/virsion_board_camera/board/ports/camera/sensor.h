...
struct _sensor
{
    ...
    int (*reset)(struct _sensor *sensor);
    int (*sleep)(struct _sensor *sensor, int enable);
    int (*read_reg)(struct _sensor *sensor, uint16_t reg_addr);
    int (*write_reg)(struct _sensor *sensor, uint16_t reg_addr, uint16_t reg_data);
    int (*set_pixformat)(struct _sensor *sensor, pixformat_t pixformat);
    int (*set_framesize)(struct _sensor *sensor, framesize_t framesize);
    int (*set_framerate)(struct _sensor *sensor, int framerate);
    int (*set_contrast)(struct _sensor *sensor, int level);
    int (*set_brightness)(struct _sensor *sensor, int level);
    int (*set_saturation)(struct _sensor *sensor, int level);
    int (*set_gainceiling)(struct _sensor *sensor, gainceiling_t gainceiling);
    int (*set_quality)(struct _sensor *sensor, int quality);
    int (*set_colorbar)(struct _sensor *sensor, int enable);
    int (*set_auto_gain)(struct _sensor *sensor, int enable, float gain_db, float gain_db_ceiling);
    int (*get_gain_db)(struct _sensor *sensor, float *gain_db);
    int (*set_auto_exposure)(struct _sensor *sensor, int enable, int exposure_us);
    int (*get_exposure_us)(struct _sensor *sensor, int *exposure_us);
    int (*set_auto_whitebal)(struct _sensor *sensor, int enable, float r_gain_db, float g_gain_db, float b_gain_db);
    int (*get_rgb_gain_db)(struct _sensor *sensor, float *r_gain_db, float *g_gain_db, float *b_gain_db);
    int (*set_auto_blc)(struct _sensor *sensor, int enable, int *regs);
    int (*get_blc_regs)(struct _sensor *sensor, int *regs);
    int (*set_hmirror)(struct _sensor *sensor, int enable);
    int (*set_vflip)(struct _sensor *sensor, int enable);
    int (*set_special_effect)(struct _sensor *sensor, sde_t sde);
    int (*set_lens_correction)(struct _sensor *sensor, int enable, int radi, int coef);
    int (*ioctl)(struct _sensor *sensor, int request, va_list ap);
    int (*snapshot)(struct _sensor *sensor, uint8_t *image, uint32_t flags);
};
